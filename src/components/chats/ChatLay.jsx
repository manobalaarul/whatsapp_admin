...
const ChatLay = () => {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await Axios({
        method: 'POST',
        url: SummaryApi.send_message.url, // Add this to your SummaryApi
        data: {
          client: activeUser.clientId,
          message: newMessage,
          admin: 'your_admin_id' // Replace with actual admin ID
        }
      });

      if (response.data.success) {
        // Message will be added via SSE, so just clear input
        setNewMessage("");
      }
    } catch (error) {
